  ...
  if (canRenderImage) {
    return (
      <div className={cn("space-y-3", className)}>
        <div
          className={cn(
            "overflow-hidden rounded-[2rem] border border-[color:var(--line)] bg-[rgba(255,255,255,0.03)] shadow-[0_24px_80px_rgba(0,0,0,0.28)]",
        ...
        >
          <div className="relative h-full w-full">
            <Image src={imageSrc} alt={alt ?? label} fill className="object-contain" />
          </div>
        </div>
        {caption ? <p className="text-sm leading-7 text-[var(--text-soft)]">{caption}</p> : null}
      </div>
    );
  }
